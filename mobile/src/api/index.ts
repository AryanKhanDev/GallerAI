...
export interface ChatMessage {
  ...
  role: MessageRole;
  text: string;
  images?: ImageResult[];       // retrieved images attached to this message
  action?: "query" | "album_created" | "added_to_album" | "error";
  albumId?: string;             // if action=album_created
  latency_ms?: number;
}
...
interface GalleryState {
  images: GalleryImage[];
  total: number;
  activeTag: string;            // current tag filter ("" = no filter)
  tagScores: Record<string, number>; // imageId → relevance score for activeTag
